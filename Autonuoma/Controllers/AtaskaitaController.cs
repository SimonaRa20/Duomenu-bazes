...
namespace Org.Ktu.Isk.P175B602.Autonuoma.Controllers
{
    ...
    public class AtaskaitaController : Controller
    {
        ...
        public ActionResult Paslaugos(DateTime? dateFrom, DateTime? dateTo, string publisher)
        {
            var report = AtaskaitaRepo.GetTotalServicesOrdered(dateFrom, dateTo, publisher);
            report.DateFrom = dateFrom;
            report.DateTo = dateTo?.AddHours(23).AddMinutes(59).AddSeconds(59); //move time of end date to end of day

            report.Paslaugos = AtaskaitaRepo.GetServicesOrdered(dateFrom, dateTo, publisher);
